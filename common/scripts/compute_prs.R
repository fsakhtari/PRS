...
opts_spec <- list(
  make_option(c("--chr"), type="integer", default=NULL, 
              help="Chromosome # for which to compute PRS", metavar="integer"),

  make_option(c("--summstats_matched"), type="character", default=paste0(PRS_PHENO_DIR, '/derived_data/summary_stats/summstats_matched.txt'), 
              help="Matched summary statistics file", metavar="character"),
  ...
  make_option(c("--validn_pheno"), type="character", default=paste0(PRS_PHENO_DIR,'/derived_data/validation_data/ukb_pheno.rds'), 
            help="Validation data .rds file containing the phenotype & covariates [default \"%default\"]", metavar="character"),
...
print(sprintf("Chromosome %d : Number of SNPs in matched summary statistics = %d", opts$chr, length(ind.chr.ss)))
...
corr_mat <- snp_cor(ldref.bigSNP$genotypes, ind.col = ind.chr.ldref, ncores = NCORES, infos.pos = as.vector(unlist(matched_summstats[ind.chr.ss, 'genetic.pos'])), size = 3 / 1000, alpha = 0.9)
corr_sfbm <- bigsparser::as_SFBM(as(corr_mat, "dgCMatrix"))

cat("Chromosome", opts$chr, ": Dimensions of SNP correlation matrix = ", dim(corr_mat), "\n")
if (anyNA(corr_mat)) {
...
## LDpred2 - Infer posterior mean effect size for each SNP by using a prior on effect sizes and LD information from the LD reference panel

# Adjust betas for each parameter combination
ldref.beta_grid <- snp_ldpred2_grid(corr = corr_sfbm, df_beta = df_beta, grid_param = params, ncores = NCORES)

write.table(ldref.beta_grid, paste0(out_dir, 'ldref_adjbetas_na_chr', opts$chr, '.txt'), quote = FALSE, row.names = FALSE)

# ldpred2 fails for some parameter combinations and returns all NA values. Remove these invalid values.
...
# ldpred2 returns absurd/extreme beta values for certain parameter combinations. Remove these absurd, extreme values and keep the sane values.
std_devs <- apply(ldref.beta_grid, 2, sd)
sane <- (1:ncol(ldref.beta_grid))[abs(std_devs - median(std_devs)) < 3 * mad(std_devs)]

cat("Number of parameter combinations that gave insane beta values =", length(setdiff(1:nrow(params), sane)), "\n")
...
write.table(ldref.beta_grid, paste0(out_dir, 'ldref_adjbetas_chr', opts$chr, '.txt'), quote = FALSE, row.names = FALSE)
...
validn.beta_grid <- apply(ldref.beta_grid, 2, function(beta_vec) return( beta_vec * matched_summstats$beta.validn[ind.chr.ss]))

# Compute PRS
pred_grid <- big_prodMat(validn.bigSNP$genotypes, validn.beta_grid, ind.col = matched_summstats$`_NUM_ID_.validn`[ind.chr.ss])

write.table(pred_grid, paste0(out_dir, 'validn_prsgrid_chr', opts$chr, '.txt'), quote = FALSE, row.names = FALSE)
...
# checks: if highest AUC is the one with the min p, min h or max h, then print a warning + suggest changing hyperparameters
# info: save whole param table with auc. save best param values for chr. merge for all chrs and make plots, print ranges, etc.

write.table(logreg_op, paste0(out_dir, 'validn_glm_chr', opts$chr, '.txt'), quote = FALSE, row.names = FALSE)

# Plots and tables to check for convergence of hyperparameters

# The rows in params and logreg are in the same order as the columns in *.beta_grid and pred_grid
...
write.table(set_names(data.frame(validn.best_prs), paste0('chr', opts$chr)), paste0(out_dir, 'validn_prs_chr', opts$chr, '.txt'), quote = FALSE, row.names = FALSE)


## Compute PRS in the test data using the best adjusted beta vector

# Extract the best adjusted beta vector that corresponds to the best params, i.e. the PRS with the highest AUC. Here, we must extract the appropriate column from ldref.beta_grid because the signs for the betas in the test data (beta.test from snp_match()) are relative to the LD reference panel.
...
write.table(set_names(data.frame(test.beta_vec), paste0('chr', opts$chr)), paste0(out_dir, 'test_bestbetas_chr', opts$chr, '.txt'), quote = FALSE, row.names = FALSE)

# Compute PRS
prs_test <- big_prodVec(test.bigSNP$genotypes, test.beta_vec, ind.col = matched_summstats$`_NUM_ID_.test`[ind.chr.ss])

write.table(set_names(data.frame(prs_test), paste0('chr', opts$chr)), paste0(out_dir, 'test_prs_chr', opts$chr, '.txt'), quote = FALSE, row.names = FALSE)
...
# ## Automatic model
# # Adjust betas using the Automatic model which automatically estimates hyperparameters
...
# # TODO: add error checking + convergence checks of p_est, h2_est values for each list in multi-auto - see ldpred2 tutorial, compute_prs_auto.R & Downloads/random_ldpred.R

# # Extract the beta values for each estimated parameter combination in multi_auto. These betas are appropriate for the genotypes/alleles in the LD reference panel. Number of columns in the beta grid is 2xlength(multi_auto). The first n columns are for sparse=FALSE and the remaining n columns are for sparse=TRUE. These beta

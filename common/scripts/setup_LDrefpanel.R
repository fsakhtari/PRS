...
# 1000G Phase3 data downloaded via bigsnpr, for 2490 mostly unrelated individuals and ~1.7M SNPs in common with either HapMap3 or the UK Biobank.
...
bedfile <- snp_plinkQC(plink.path = '/ddn/gs1/home/akhtarifs/software/plink2',prefix.in = paste0(ldref_in_dir, '1000G_phase3_common_norel'),
  prefix.out = paste0(ldref_out_dir, '1000g_phase3_geno'),
  maf = 0.01, geno = 0.01, mind = 0.1, hwe = 1e-10, autosome.only = TRUE, extra.options = " --snps-only")
...
popn_info <- dplyr::left_join(popn_info, ped, by = c("sample.ID" = "Individual ID"))
popns <- bigreadr::fread2(paste0(ldref_in_dir, '20131219.populations.tsv'))
popn_info <- dplyr::left_join(popn_info, popns, by = c("Population" = "Population Code"))
popn_info$popn_code <- paste(popn_info$`Super Population`, popn_info$Population, sep = "_")
write.table(popn_info, paste0(ldref_out_dir, "1000g_phase3_geno_popinfo.txt"), row.names = FALSE)

# Relatedness
# https://cran.r-project.org/web/packages/bigsnpr/bigsnpr.pdf states that these are mostly unrelated. Upon examination, this subset does seem mostly unrelated, i.e. one of a pair of close relatives has been removed. 
# However, if you only want to keep individuals who have Relationship == 'unrel', then the following code will keep 907 samples from the 2490.

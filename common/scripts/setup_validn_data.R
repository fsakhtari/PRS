...
opts_spec <- list(
  make_option(c("--pheno"), type="character", default=NULL, 
              help="Phenotype for which to compute PRS", metavar="character"),

  make_option(c("--covariates"), type="list", default=c(list(age='f.21022.0.0', sex='f.31.0.0', ethnicity='f.21000.0.0', bmi='f.21001.0.0'), pcs),
    help="covariates to use in the association analysis specified in a named list of covariate names and their corresponding UIDs in the UK Biobank data [default \"%default\"]", metavar="named list")
);
...
ukb_data <- read.table("/ddn/gs1/shared/ukbiobank/dataformat_R/ukb41671.tab", header=TRUE, sep="\t")
print("UK Biobank (UKB) survey data :")
cat("dimensions =", dim(ukb_data))
print("first 50 column names :")
print(colnames(ukb_data)[1:50])
saveRDS(ukb_data, "/ddn/gs1/project/controlled/EPR/EPR_statgen/akhtarifs/tmp/dataformat_R/ukb41671.rds")
...
ukb_survey_cc <- ukb_data %>%
  ...
  replace_with_na_at(.vars = c("Y", "ethnicity"), condition = ~.x %in% na_strings) %>%
  drop_na() %>%
  # Group the ethnicities into the top-level ethnic group which is represented by the first number in the string
  ...
  #  %>%
  # # TODO: temporary removal of samples/individuals to decrease runtime
  # sample_n(10000)
...
reldness <- read.table('/ddn/gs1/shared/ukbiobank/genotype_data/common_data/ukb57849_rel_s488264.dat', header = TRUE)
indivs.remove <- ukb_gen_samples_to_remove(data = reldness, ukb_with_data = ukb_survey_cc$f.eid, cutoff = 0.0884)
cat("Number of related individuals to remove: ", length(indivs.remove), "\n")

ukb_survey_cc <- ukb_survey_cc %>% anti_join(data.frame(f.eid = indivs.remove), by = 'f.eid')
...
write.table(data.frame(FID = ukb_survey_cc$f.eid, IID = ukb_survey_cc$f.eid), file = indivs_keep, quote = FALSE, row.names = FALSE, col.names = FALSE)


## Genotype data

# Filter genotype data + only keep genotype samples for complete cases
bedfile <- snp_plinkQC(plink.path = '/ddn/gs1/home/akhtarifs/software/plink2',prefix.in = "/ddn/gs1/shared/ukbiobank/genotype_data/genotypes/all_chromosomes/ukb_cal_all_v2",
  prefix.out = paste0(valdn_out_dir, "ukb_geno"),
  maf = 0.01, geno = 0.01, mind = 0.1, hwe = 1e-10, autosome.only = TRUE,extra.options = paste0(" --snps-only --keep ", indivs_keep))
...
validn.bigSNP$genotypes <- snp_fastImputeSimple(validn.bigSNP$genotypes, method= "random")

...
opts_spec <- list(
  make_option(c("--pheno"),
    type = "character",
    default = NULL,
    help = "Variable name in the EPR H&E Survey data of the phenotype for which
    to compute PRS",
    metavar = "character"
  )
)
...
test_prs <- read.table(paste0(PRS_PHENO_DIR, "/output/test_prs_df.txt"),
  header
  = TRUE
)
...
get_all_exps <- function() {
  ...
  he_drop_vars <- qc(
    he_a001a_height_ft, he_a001b_height_in,
    he_a001c_height_in_derived, he_a002_weight, he_bmi_derived,
    he_bmi_cat_derived
  )

  he_exp_vars <- epr.he.meta %>%
    filter(survey_section %in% c(
      "About Your General Health", "Occupation",
  ...
  expa_exp_vars <- epr.ea.meta %>%
    filter(!survey_section %in% c("Not Applicable",
...
for (col in colnames(prs_e)) {
  ...
  # unbalanced data
  aov_obj <- aov(as.formula(paste("PRS ~", col)), data = test_df,
                 singular.ok = FALSE)
  print(aov_obj)
  ...
  # lm_obj <- lm(as.formula(paste('PRS ~', col)), data = test_df,
    # singular.ok = FALSE)
  # print(lm_obj)
  # summ <- summary(lm_obj)
  # print(summ)
  ...
  all_res <- rbind(all_res, data.frame(exposure = col, pvalue = pval,
                                       n = nobs(aov_obj)))
...
# MTC
all_res$pvalue_adj <- p.adjust(all_res$pvalue, method = "fdr")
...
# # for now assuming that variables with < 10 levels are factors/categorical.
# Could switch to only doing this for factor variables, if variable types are

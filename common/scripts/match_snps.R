...
opts_spec <- list(
  make_option(c("--summstats_fmtd"),
    type = "character",
    default = paste0(PRS_PHENO_DIR, "/derived_data/summary_stats/summstats_fmtd.txt"),
    help = "Formatted summary statistics file [default \"%default\"]",
    metavar = "character"
  ),
...
common_snps <- Reduce(intersect,
    list(ldref.map$rsid, validn.map$rsid, test.map$rsid, summstats$rsid)
  )
...
SD.validn <- data.frame(
  rsid = validn.bigSNP$map$marker.ID,
  SDval = sqrt(big_colstats(validn.bigSNP$genotypes)$var)
)

SD.test <- data.frame(
  rsid = test.bigSNP$map$marker.ID,
  SDtest = sqrt(big_colstats(test.bigSNP$genotypes)$var)
)

SD.all <- Reduce(function(x, y) merge(x, y, by = "rsid"),
    list(SD.ss, SD.validn, SD.test)
  )
print("Standard deviations of SNPs in summary statistics, validation and test datasets:")
print(str(SD.all))

# For each SNP (row), compare standard deviations across summary statistics,
# validation & test datasets and flag violations
SD.all$keep <- apply(SD.all %>% select(-rsid), 1, check_sd)
write.table(
  SD.all, paste0(ss_dir, "SD_all.txt"),
...
ggplot(SD.all, aes(x = SDval, y = SDss, colour = keep)) +
  ...
  labs(
    x = "Standard deviations in the validation dataset",
...
ggplot(SD.all, aes(x = SDtest, y = SDss, colour = keep)) +
  ...
  geom_abline(intercept = 0, slope = 1, linetype = 2, colour = "Red") +
  labs(
    x = "Standard deviations in the test dataset",
    y = "Standard deviations derived from the summary statistics"
  ) +
  theme_bw()
...
# Remove the flagged SNPs from the summary statistics. Only the SNPs remaining
# in the summary statistics will be used for further analysis.
snps.keep <- SD.all %>%
  filter(keep == TRUE) %>%
  pull(rsid)
...
info_snp.ld <- rename(info_snp.ld, c(
  "_NUM_ID_.summstats" = "_NUM_ID_.ss",
...
info_snp.validn <- snp_match(info_snp.ld, validn.map,
  join_by_pos = FALSE,
  match.min.prop = 0
)
str(info_snp.validn)
...
final_info <- inner_join(info_snp.validn, info_snp.test,
  by = setdiff(
    c(names(info_snp.ld), "_NUM_ID_.ss"),
    c("beta", "_NUM_ID_", "pos")
  ),
...
# Use genetic maps to interpolate physical positions (in bp) to genetic
# positions (in cM).
# TODO: GRCh/hg versions need to be same for the LDrefpanel (we are using $pos
# from there) and for whatever snp snp_asGeneticPos is using.
final_info$genetic.pos <- snp_asGeneticPos(
  final_info$chr, final_info$pos.ldref,
  dir = paste0(PRS_COMMON_DIR, "/raw_data/1000G_genetic_maps"), ncores = NCORES)

# Save the matched SNPs & info across all 4 datasets
write.table(
  final_info, file = paste0(ss_dir, "summstats_matched.txt"),
